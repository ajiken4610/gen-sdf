...
export default (
  ...
  return new Promise<{ canvas: HTMLCanvasElement; width: number; height: number }>(async (resolve) => {
    ...
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0, 0, ${vw}, ${vh}" width="${
      options.width
    }" height="${options.height}"${!options.originalRatio ? ' preserveAspectRatio="none"' : ""}>
